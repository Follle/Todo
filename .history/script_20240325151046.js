...
const btnActive = document.getElementById('completed');
...
function countCheck(){// считает выбранные и не очень 
    ...
    btnActive.textContent = `Completed(${tasksArray.length-notCheckAllCount})`;
    //tasksList.innerHTML += `<label>${"  " + "All" + "(" + (checkAllCount+notCheckAllCount) + ")" }</label>`;  
    tasksList.innerHTML += `<label>${"Completed" + "(" + checkAllCount + ")" + "  "}</label>` ;
    tasksList.innerHTML += `<label>${"  " + "Active" + "(" + notCheckAllCount + ")" + "     "}</label>`;     
