...
function render() {// выводит инфу
    ...
    tasksList.innerHTML= "";
    paginatedData.forEach((elem) => tasksList.innerHTML += `<div id="${elem.id}">
...
function paginationButtons() {
    pag.innerHTML = ``;
    totalPages = Math.ceil(tasksArray.length/rows);
    for (let i = 0; i < totalPages; i += 1) {
        pag.innerHTML += `<button id="${i}">${i+1}</button>`;
    }
}

function paginationB(event){
...
pag.addEventListener('click', paginationB);
