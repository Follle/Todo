...
function countCheck(){// считает выбранные и не очень 
    ...
    tasksList.innerHTML += `</div><label>${"A" + checkAllCount}</label>`</div><label>${"A" + notCheckAllCount}</label>`;
    tasksList.innerHTML += `</div><label>${"A" + notCheckAllCount}</label>`;     
    tasksList.innerHTML += `</div><label>${"A" + (checkAllCount+notCheckAllCount)}</label>`;  
    //document.getElementById('task-list-completed').innerHTML = `<p>${checkAllCount}</p>`;
    //document.getElementById('task-list-completed').innerHTML = `<div>${checkAllCount}</div>`;
