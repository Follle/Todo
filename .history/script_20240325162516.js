...
function pagination(){
    const notesOnPage = 5; //количество на одной стр
    let currentUser = 0; // счетчик страниц
    const items = tasksArray.from(content.getElementsByTagName('div')).slice(1);
    createPageButtons(); // Call this function to create the page buttons initially
  showPage(currentPage);
}

function showPage(page){
    const startIndex = page * notesOnPage;
    const endIndex = startIndex + notesOnPage;
    tasks.forEach((task, index) => {
      task.classList.toggle('hidden', index < startIndex || index >= endIndex);
    });
...
function createPageButtons() {
    const totalPages = Math.ceil(tasks.length / notesOnPage);
