...
function render() {// выводит инфу
    ...
    //countCheck();
    //checkAllCount.innerHTML = `<div>${checkAllCount}</div>`;
    //document.getElementById("checkAllCount").innerHTML = checkAllCount; 
    //document.getElementById("notCheckAllCount").innerHTML = notCheckAllCount;
...
function countCheck(){// считает выбранные и не очень 
    ...
    checkAllCount.forEach((elem) => tasksList.innerHTML += `<div id="${elem.id}">
    <input type="checkbox" ${elem.isChecked ? 'checked' : ''}><label>${elem.text}</label>
    <button>X</button>
    </div>`);
     яcheckAllCount.innerHTML = `<div>${checkAllCount}</div>`;
