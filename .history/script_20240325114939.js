...
const showAllActive = document.querySelector('.i-5');
...
function chooseTaskStatus(filter){
    let tempArray;
    filteredTasks.forEach(task => {
    console.log(`${elem.id}: ${telem.title} (${elem.isChecked ? 'Выполнено' : 'Активно'})`);
    });
    

    // Получение типа отображаемых задач от пользователя
    let filter = prompt('Выберите тип отображаемых задач: All, Active, Completed', 'All');
...
//function chooseTaskCompleted(event){//оставляет только готовые
    //const tempArray = tasksArray.filter((item) => item.isChecked)
   // render();
//}

// function chooseTaskActive(){
//     tasksArray = tasksArray.filter((item) => !item.isChecked)
//     render();
// }
...
//showAllActive.addEventListener('click',chooseTaskActive);
//showAllCompleted.addEventListener('click',chooseTaskCompleted);
