...
function render() {// выводит инфу
    ...
    document.querySelectorAll('div .tab-btn')
    ...
    //document.querySelector('.tab-btn').textContent="All("+ checkAllCount+")";
}
...
function countCheck(){// считает выбранные и не очень 
    ...
    tasksList.innerHTML += `</div><label>${"Completed" + "(" + checkAllCount + ")" + "  "}</label>` ;
    tasksList.innerHTML += `</div><label>${"  " + "Active" + "(" + notCheckAllCount + ")" + "     "}</label>`;     
...
function chooseTaskStatus(event){
    ...
    switch(event.target.id) {
        case 'all': 
            countCheck();
            showStatus.textContent += `<div></div><button>"All("+(checkAllCount+notCheckAllCount)+")"</button>`;
            += `</div><label>${"  " + "All" + "(" + (checkAllCount+notCheckAllCount) + ")" }</label>`;
